...
local ufoClasses = {
  -- saucer
  {scores={50,100,150}, special=300, w=16, h=7, color={1,0,1}, phaser=false, phasePeriod=0,
   shipBmp="saucer", boomBmp="ufoboom"},
  -- schrodinger
  {scores={300,350,400}, special=1000, w=15, h=7, color={0,1,1}, phaser=true, phasePeriod=SCHRODINGER_PHASE_PERIOD,
   shipBmp="schrodinger", boomBmp="ufoboom"},
}
local SAUCER=1; local SCHRODINGER=2

local bombClasses = {
  -- cross
  {w=3, h=6, speed=-80, colorIdx=1, frameInterval=20, laserSurvive=0,
   bmp={"cross0","cross1","cross2","cross3"}},
  -- zigzag
  {w=3, h=7, speed=-120, colorIdx=4, frameInterval=20, laserSurvive=10,
   bmp={"zigzag0","zigzag1","zigzag2","zigzag3"}},
  -- zagzig
  {w=3, h=7, speed=-100, colorIdx=5, frameInterval=20, laserSurvive=4,
...
function Game:startNextLevel()
  ...
  self.cannon = {
    alive=false, booming=false,
    x=self.worldLeftBorderX, y=ALIEN_INVASION_Y,
    w=13, h=8,
    speed=CANNON_SPEED,
    boomClock=0, boomDur=1.0,
    boomFrameClock=0, boomFrameDur=0.2, boomFrame=1,
  }
...
function Game:doInvasionTest()
  if self.isGameOver or self.alienPop == 0 or self.isAliensFrozen
     or self.isAliensSpawning or self.isAliensDropping then return end
...
function Game:doAlienBombing(beats)
  ...
  local alien = nil
  for r = 1, GRID_H do
    if self.grid[r][col].alive then alien = self.grid[r][col]; break end
  end
...
function Game:doBombBoomBooming(dt)
  for i = 1, MAX_BOMBS do
    ...
    if boom.alive then
      ...
      if boom.clock <= 0 then boom.alive = false end
    end
  end
end
...
function Game:doCollisionsBombsHitbar()
  for i = 1, MAX_BOMBS do
    ...
    if bomb.y > self.hitbarY then goto continue end
...
function Game:doCollisionsBombsLaser()
  ...
  for i = 1, MAX_BOMBS do
    ...
    if hit then
      self:boomLaser(true)
      -- Show explosion at collision point but don't destroy the bomb
      self:spawnBoom(bomb.x, bomb.y, BOMBHIT_MIDAIR, bc.colorIdx)
      -- Only destroy bomb based on probability
...
function Game:doCollisionsLaserAliens()
  if not self.laser.alive or self.isAliensSpawning or self.isAliensFrozen
     or self.alienPop == 0 then return end
  local lBmp = self.bitmaps["laser0"]
  for r = 1, GRID_H do
    for c = 1, GRID_W do
      local alien = self.grid[r][c]
      if not alien.alive then goto continue end
      local ac   = alienClasses[alien.classId]
...
function Game:doCollisionsBunkersBombs()
  ...
  for i = 1, MAX_BOMBS do
    local bomb = self.bombs[i]
    if not bomb.alive then goto nextBomb end
    if bomb.y < BUNKER_SPAWN_Y or bomb.y > BUNKER_SPAWN_Y + 16 then goto nextBomb end
    local bc   = bombClasses[bomb.classId]
    local bBmp = self.bitmaps[bc.bmp[bomb.frame]]
    for bi = #self.bunkers, 1, -1 do
      local bunker = self.bunkers[bi]
      if not bunker.alive then goto nextBunker end
...
function Game:doCollisionsBunkersLaser()
  ...
  for bi = #self.bunkers, 1, -1 do
    local bunker = self.bunkers[bi]
    if not bunker.alive then goto continue end
...
function Game:doCollisionsBunkersAliens()
  ...
  local bottomRow = 0
  for r = 1, GRID_H do
    if self.rowPops[r] > 0 then bottomRow = r; break end
  end
  ...
  for c = 1, GRID_W do
    ...
    if not alien.alive then goto nextAlien end
    local ac   = alienClasses[alien.classId]
    local aBmp = self.bitmaps[ac.bmp[alien.frame]]
    for bi = #self.bunkers, 1, -1 do
      local bunker = self.bunkers[bi]
